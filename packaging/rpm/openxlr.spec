...
- Tooltips sit against the control they describe and flip to the side of it that has room, so one near a screen edge no longer covers the button or takes the click meant for it.
- The window repairs its autostart entry only where a repair is needed and only once per installation path, leaving an edited command, a Hidden disable or a symbolic link alone, and Options keeps the previous preference when a change fails.
- Sends, mutes and saved settings survive failures: a send whose combine leg arrives late is applied when it does, a missing one is retried without being given up, settings are written at shutdown, a failed write is reported once, and a recalled profile sets only the sends it names.
- A plugin that stops passing audio is replaced the way a crashed one is, the plugin output the daemon reads is bounded, CLAP plugins are described from both bus layouts, and installing over a plugin already in place swaps in the new copy only once it is complete.
- Inserts resolve their plugin against everything installed, so a chain whose plugin did not fit in the bounded list still loads and reports, and that list now puts the plugins the saved chains use first.
- Bundles a scan could not read are named in install, sync and rescan replies and logged with their outcome and exit code, retained scanner output keeps its beginning and its end, and a cached CLAP or VST3 description is reused only while the helper that wrote it is the one asking.
- The daemon ends the Wine session a bridged plugin leaves behind once the last bridged insert stops and when the daemon stops, and the user service sends SIGTERM to the daemon alone, so a stop takes seconds instead of the service stop timeout.
...
- Windows VST3 and CLAP plugins run as inserts through yabridge. The packages can provide a pinned companion build of it, which on Wine 9.22 or newer is what makes a bridged plugin's editor answer the mouse at all.
- A plugin's own editor takes the mouse from the first click and keeps the size the plugin asks for.
- A device that keeps no settings is given its gain back when it connects even when the gain lock is set, so it no longer comes up at the gain its firmware chooses.
