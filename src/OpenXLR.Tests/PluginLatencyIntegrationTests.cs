...
{
    [MonitorPipeWireFact]
    public void SettingsRecallUsesTheSameLatencyHostsAsTheLiveSwitch()
    {
        ...
        try
        {
            Assert.Contains(PluginCatalog.Refresh(), p => p.Plugin == "urn:openxlr:test:latency");
            ...
            string[] keys = new[] { "mix:monitor", "software", "capture" }.Where(mixer.IsInsertKey).ToArray();
            foreach (string key in keys)
                mixer.SetInserts(key, [new() { Id = "delay", Kind = "lv2", Plugin = "urn:openxlr:test:latency" }]);
            ...
            void AssertNativeHosts()
            {
                foreach (string key in keys)
                {
                    var insert = Assert.Single(mixer.Snapshot().Inserts[key]);
                    Assert.Null(insert.Error);
                    Assert.True(insert.NativeHostRunning, key + " must use a latency-reporting host after either activation path.");
                }
            }
        }
        finally
    ...
    public async Task LiveLv2LatencyChangesBypassAndProcessFailureRecalculateTheOtherMixes()
    {
        ...
        var pw = new PipeWireAdapter(() => nearLimit ? (990, 1024) : (0, 4096));
        using var registry = pw.WatchGraph();
        using var mixer = new Mixer(pw);
        ...
        try
        {
            ...
            {
                mixer.EnsureFilterRoutes();
                return mixer.Snapshot().MixLatencyError?.Contains("open-file limit", StringComparison.Ordinal) == true;
            }, TimeSpan.FromSeconds(5)));
            Assert.Null(pw.FindNodeId("OpenXLR_delay_monitor_in"));
            Assert.Throws<InvalidOperationException>(() => mixer.CreateVirtualMix("Extra", _ => null));
            Assert.Equal(2, mixer.Config.Mixes.Count);
            nearLimit = false;
            Assert.True(SpinWait.SpinUntil(() =>
            {
                mixer.EnsureFilterRoutes();
        ...
        void WaitForDelay(double value)
        {
            Assert.True(SpinWait.SpinUntil(() =>
            {
                ...
                return state.MixLatencyError is null && state.MixDelayMilliseconds.GetValueOrDefault("monitor", 0) == value;
            ...
            Assert.Equal(0, mixer.Snapshot().MixDelayMilliseconds.GetValueOrDefault("chat", 0));
        }
    }
    ...
    [MonitorPipeWireFact]
    public void FailedRewireAndFailedSettingsRollbackReportBothErrorsAndKeepTheSavedFlag()
    {
        var pw = new PipeWireAdapter((Func<DspFeatureAvailability>)(() => throw new IOException("DSP rewire failure")));
        using var registry = pw.WatchGraph();
        using var mixer = new Mixer(pw);
        pw.CreateNullSink("latency_failure_signal", "Signal");
        pw.CreateVirtualMic("Wave_XLR_latency_failure", "latency_failure_signal.monitor", "Synthetic microphone");
        mixer.SetInputDeviceHint("Wave_XLR_latency_failure");
        mixer.Build(new MixerConfig { Channels = [new("xlr1", "Microphone") { InputPair = 0 }], Mixes = [new("monitor", "Monitor", MixKind.Monitor)] });
        typeof(Mixer).GetField("_softClipGuard", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic)!.SetValue(mixer, true);
        int saves = 0;
        var failure = Assert.Throws<AggregateException>(() => mixer.SetMixLatencyCompensation(true, settings =>
        {
            saves++;
            if (saves == 1) { Assert.True(settings.CompensateMixLatency); return null; }
            Assert.False(settings.CompensateMixLatency);
            throw new IOException("rollback disk failure");
        }));
        Assert.Contains(failure.InnerExceptions, e => e.Message.Contains("DSP rewire failure", StringComparison.Ordinal));
        Assert.Contains(failure.InnerExceptions, e => e.Message.Contains("rollback disk failure", StringComparison.Ordinal));
        Assert.Equal(2, saves);
        Assert.True(mixer.ExportSettings().CompensateMixLatency);
        Assert.True(pw.MeasurePluginLatency);
    }
