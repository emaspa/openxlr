...
public sealed class ChannelInsertIntegrationTests
{
    ...
    [MonitorPipeWireFact]
    public async Task SoftwareAndCaptureChainsProcessBothSidesAndRestoreDirectSends()
    {
        ...
        {
            AssertSound(1);
            Assert.DoesNotContain(pw.DumpNodes(), n => n.Name.StartsWith("OpenXLR_bus_", StringComparison.Ordinal));
            ...
            AssertSound(.25);
            int? beforeRecall = pw.FindNodeId(failed.Name);
    ...
    [MonitorPipeWireFact]
    public void EmptySoftwareAndCaptureChainsRetainTheOriginalNodeSet()
    {
        var pw = new PipeWireAdapter();
        using var registry = pw.WatchGraph();
        using var mixer = new Mixer(pw);
        mixer.Build(new MixerConfig { Channels = [new("game", "Game"), new("capture", "Capture") { CaptureSource = "missing" }], Mixes = [new("monitor", "Monitor", MixKind.Monitor)] });
        string[] before = pw.DumpNodes().Where(n => n.Name.StartsWith("OpenXLR", StringComparison.Ordinal)).Select(n => n.Name).Order().ToArray();
        Assert.Contains("OpenXLR_ch_game", before);
        Assert.Contains("OpenXLR_ch_capture", before);
        Assert.DoesNotContain(before, name => name.StartsWith("OpenXLR_bus_", StringComparison.Ordinal));
        mixer.SetInserts("game", []);
        mixer.SetInserts("capture", []);
        Assert.False(mixer.EnsureFilterRoutes());
        Assert.Equal(before, pw.DumpNodes().Where(n => n.Name.StartsWith("OpenXLR", StringComparison.Ordinal)).Select(n => n.Name).Order().ToArray());
    }

    [MonitorPipeWireFact]
    public void FailedChannelLinksRecoverWithAHealthyDirectFallback() => CheckFailedChannelLinks(false);

    [MonitorPipeWireFact]
    public void FailedChannelLinksRecoverAfterBothPathsFail() => CheckFailedChannelLinks(true);

    private static void CheckFailedChannelLinks(bool failDirectRoute)
    {
        Assert.Contains(PluginCatalog.Refresh(), p => p.Plugin == Plugin);
        string directory = Directory.CreateTempSubdirectory("openxlr-channel-link-fault-").FullName;
        string? oldPath = Environment.GetEnvironmentVariable("PATH");
        string realLink = oldPath!.Split(Path.PathSeparator).Select(p => Path.Combine(p, "pw-link")).First(File.Exists);
        string fault = Path.Combine(directory, "fail");
        var pw = new PipeWireAdapter();
        using var registry = pw.WatchGraph();
        using var mixer = new Mixer(pw);
        try
        {
            ...
            ExecutableScript.Write(Path.Combine(directory, "pw-link"), $$"""
                if [ -f "${0%/*}/fail" ] && [ "$1" != "-d" ]; then
                    case "$*" in
                        *OpenXLR_ins_channel_software_*) exit 1;;
                        *OpenXLR_bus_software*) if [ '{{failDirectRoute}}' = True ]; then exit 1; fi;;
                    esac
                fi
                exec '{{realLink}}' "$@"
                """);
            Environment.SetEnvironmentVariable("PATH", directory + Path.PathSeparator + oldPath);
            mixer.ApplySettings(mixer.ExportSettings() with
            {
                Inserts = new() { ["software"] = [Gain(.5)], ["other"] = [Gain(.25)] },
            });
            Assert.NotNull(Assert.Single(mixer.Snapshot().Inserts["software"]).Error);
            Assert.Null(Assert.Single(mixer.Snapshot().Inserts["other"]).Error);
            var otherNodes = pw.DumpNodes().Where(n => n.Name.StartsWith("OpenXLR_ins_channel_other_", StringComparison.Ordinal)).Select(n => n.Id).Order().ToArray();
            Assert.NotEmpty(otherNodes);
            File.Delete(fault);
            Assert.True(SpinWait.SpinUntil(() =>
            {
                mixer.EnsureFilterRoutes();
                return Assert.Single(mixer.Snapshot().Inserts["software"]).Error is null;
            }, TimeSpan.FromSeconds(8)), "The processing chain should recover even when its direct fallback is healthy.");
            Assert.Equal(otherNodes, pw.DumpNodes().Where(n => n.Name.StartsWith("OpenXLR_ins_channel_other_", StringComparison.Ordinal)).Select(n => n.Id).Order().ToArray());
        }
        finally
        {
