...
{
    [Fact]
    public void MissingHelperExplainsSoundCheckInsteadOfAnInsertHostSwitch()
    {
        string path = NativePluginHost.Executable;
        string backup = path + ".soundcheck-test-backup";
        bool existed = File.Exists(path);
        if (existed) File.Move(path, backup);
        try
        {
            var error = Assert.Throws<InvalidOperationException>(() => new PipeWireAdapter().CreateSoundCheck("xlr1", out _));
            Assert.Contains("Sound Check needs", error.Message);
            Assert.DoesNotContain("filter chain", error.Message);
        }
        finally { if (existed) File.Move(backup, path); }
    }
