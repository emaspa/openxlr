...
public sealed class EffectWorkflowTests : IDisposable
{
    ...
    [Fact]
    public async Task AppendingPastTheLimitExplainsHowToProceedWithoutSendingACommand()
    {
        await using var client = new DaemonClient();
        var view = new InsertsViewModel(client, "xlr1");
        var chain = new JsonArray();
        for (int i = 0; i < 16; i++) chain.Add(new JsonObject { ["insert"] = Chain().Copy(true).Inserts[0]!.DeepClone() });
        view.Apply(chain);
        view.CopyEffects(view.Items[0]);
        await view.PasteEffectsAsync(false);
        Assert.Contains("at most 16 effects", view.WorkflowError);
        Assert.Contains("replace", view.WorkflowError);
        Assert.Equal(16, view.Items.Count);
    }
    ...
    public async Task EditingAComparisonClearsTheListeningLabelAndKeepsItsSlot(string edit, bool duringReply)
    {
        var received = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var release = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        await using var server = await SocketTestServer.Start(async (socket, stop) =>
        {
            ...
            {
                var command = await SocketTestServer.Receive(socket, stop);
                if (command["cmd"]!.GetValue<string>() != "setInserts" || command["requestId"] is null) continue;
                received.TrySetResult();
                await release.Task.WaitAsync(stop);
                await SocketTestServer.Send(socket, new { type = "commandResult", requestId = command["requestId"]!.GetValue<string>() }, stop);
            }
        ...
        {
            release.SetResult(); await hearing;
            Assert.Equal("Hearing A", view.Comparison);
            view.Apply(state.DeepClone());
            Assert.Equal("Hearing A", view.Comparison);
        }
        var insert = view.Items[0];
        switch (edit)
        {
            case "parameter": insert.SendParam("gain", .9); break;
            case "bypass": insert.Bypass = !insert.Bypass; break;
            case "host": insert.NativeHost = !insert.NativeHost; break;
            case "remove": view.Remove(insert); break;
            case "remote": state[0]!["insert"]!["params"]!["gain"] = .9; view.Apply(state); break;
        }
        if (duringReply) { release.SetResult(); await hearing; }
        Assert.Equal("", view.Comparison);
        await view.HearComparisonAsync(false);
        Assert.Equal("Hearing A", view.Comparison);
