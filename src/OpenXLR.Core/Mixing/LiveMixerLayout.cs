...
public sealed partial class Mixer
{
    ...
    private void CreateUserChannel(string name, string? source, int pair, Func<MixerSettings, string?> persist)
    {
        ...
        lock (_gate)
        {
            ...
            EnsurePulseHeadroomLocked(newStreams: _config.Mixes.Count, newNodes: 3);
    ...
    private void ReloadChannelSinkLocked(ChannelDefinition channel, string previousName)
    {
        ...
        RemoveChannelChainLocked(id);
        if (_channelInputModules.Remove(id, out uint old)) _pw.UnloadModule(old);
        uint fresh;
        try { fresh = _pw.CreateNullSink(channel.SinkName, $"OpenXLR {channel.Name}"); }
        catch (Exception renameError)
        {
            try { fresh = _pw.CreateNullSink(channel.SinkName, oldDescription); }
    ...
    {
        _channelInputModules[channel.Id] = module;
        WireChannelChainLocked(channel);
