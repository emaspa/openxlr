...
public sealed partial class InsertsViewModel
{
    ...
    private int _effectEditRevision;
    private EffectChainData? _hearing, _pendingComparison;
    ...
    internal async Task<bool> ApplyEffectsAsync(EffectChainData data)
    {
        ...
        try
        {
            ...
            WorkflowError = error;
            if (error is null) { _hearing = null; Comparison = ""; }
    ...
    {
        var expected = _pendingComparison ?? _hearing;
        if (expected is not null && !JsonNode.DeepEquals(expected.Inserts, CaptureChain().Inserts))
            EffectEdited();
    }
    ...
    private void ResetEffectWorkflow()
    {
        _connectionEpoch++;
        _workflowBusy = false;
        _comparisonA = _comparisonB = _hearing = _pendingComparison = null;
        Comparison = "";
        WorkflowError = null;
